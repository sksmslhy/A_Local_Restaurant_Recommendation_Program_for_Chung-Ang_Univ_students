...
def calculate(): 
	...
	place_simi_co = (place_simi_review*0.4  # 리뷰 유사도 반영
					...
					+ np.repeat([df_recom['sentiment'].values], len(df_recom['sentiment']) , axis=0) * 0.08  # 감정분석 개수
	...
	data = find_simi_place(df_recom,place_simi_co_sorted_ind, input_data1, 7)
	
	answer=', '.join(data['name'].values.tolist())
	#answer = data['name'].values.tolist()
